...
Program Definition Comma2Cocone {D C: Category} (F: D ~> C): @Δ _ 1 F ↓ Δ ~> Cocone F := {|
...
Program Definition Cocone2Comma {D C: Category} (F: D ~> C): Cocone F ~> @Δ _ 1 F ↓ Δ := {|
  fobj x := {|
    Comma.source := tt;
...
Lemma comma_cocone_inv {D C: Category} (F: D ~> C): Comma2Cocone F ∘ Cocone2Comma F = id (Cocone F).
Proof.
  fun_eq x y f.
...
Canonical CommaCocone {D C: Category} (F: D ~> C): @Δ _ 1 F ↓ Δ <~> Cocone F :=
